...
def generate_orders(num_orders=150):
    ...
    for i in range(num_orders):
        ...
        window_type = random.choices(['morning', 'afternoon', 'full_day'], weights=[0.3, 0.3, 0.4])[0]
        ...
        orders.append({
            ...
            "service_time": int(random.choices([3, 5, 10, 15], weights=[0.4, 0.4, 0.15, 0.05])[0])
        })
    return orders
...
if __name__ == "__main__":
    ...
    with open(output_dir / "realistic_orders.csv", "w", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["id", "lat", "lng", "weight", "volume", "time_window_start", "time_window_end", "service_time"])
        for order in orders:
            writer.writerow([
                order["id"],
                order["location"]["lat"],
    ...
    with open(output_dir / "realistic_vehicles.csv", "w", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["id", "start_lat", "start_lng", "end_lat", "end_lng", "capacity", "shift_start", "shift_end", "speed_factor"])
